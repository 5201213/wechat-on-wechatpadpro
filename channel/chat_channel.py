...
class ChatChannel(Channel):
    ...
    def _compose_context(self, ctype: ContextType, content, **kwargs):
        ...
        if ctype == ContextType.TEXT:
            ...
            if context.get("isgroup", False):
                valuable_types = [ContextType.SHARING, ContextType.FILE, ContextType.VIDEO, ContextType.IMAGE]
    ...
    def _generate_reply(self, context: Context, reply: Reply = Reply()) -> Reply:
        ...
        if context.get("isgroup", False) and e_context.action != EventAction.CONTINUE:
            # 获取原始内容（未去除前缀的内容）
        ...
        if not e_context.is_pass():
            ...
            elif context.type == ContextType.SHARING:  # 分享信息，当成文本处理
                context["channel"] = e_context["channel"]
                # 将SHARING类型转换为TEXT类型，这样Bot可以正常处理
                context.type = ContextType.TEXT
                reply = super().build_reply_content(context.content, context)
